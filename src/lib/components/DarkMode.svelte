...
<svelte:head>
	<script>
		if (window) {
			...
			localStorage.getItem('color-theme') === 'dark' || (!('color-theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches) ? window.document.documentElement.setAttribute('data-theme', 'dark') : window.document.documentElement.removeAttribute('data-theme');
		}
	</script>
</svelte:head>

<button on:click="{toggleTheme}" aria-label="Dark mode" type="button" {...$$restProps} class="{clsx(btnClass, $$props.class)}">
